...
class Ghost:
    ...
    def draw(self):
        if (not powerup and not self.dead) or (eaten_ghost[self.id] and not self.dead):
...
def check_position(centerx, centery):
    ...
    num2 = (WIDTH // 30)
    num3 = 15
    # check colisions based on center x and center y of player +/- fudge number
    if centerx // 30 < 29:
        if direction == 0:
...
while run:
    ...
    # add to if not powerup to check if eaten ghost
    if not powerup:
    ...
    if powerup and player_circle.colliderect(pinky.rect) and not pinky.dead and not eaten_ghost[2]:
        pinky_dead = True
        eaten_ghost[2] = True
    if powerup and player_circle.colliderect(blinky.rect) and not clyde.dead and not eaten_ghost[3]:
        clyde_dead = True
        eaten_ghost[3] = True
    if powerup and player_circle.colliderect(blinky.rect) and not eaten_ghost[0] and not blinky.dead:
        if lives > 0:
            lives -= 1
            startup_counter = 0
            player_x = 450
            ...
            pinky_dead = False
            clyde_dead = False
    if powerup and player_circle.colliderect(inky.rect) and not eaten_ghost[1] and not inky.dead:
        if lives > 0:
            lives -= 1
            startup_counter = 0
            player_x = 450
            ...
            pinky_dead = False
            clyde_dead = False
    if powerup and player_circle.colliderect(pinky.rect) and not eaten_ghost[2] and not pinky.dead:
        if lives > 0:
            lives -= 1
            startup_counter = 0
            player_x = 450
...
# create 3 extras movement algorithms
